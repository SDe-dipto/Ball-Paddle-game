...
gmover=canvas.create_text(250,190,text='Game Over',state='hidden',
                          fill='purple',font=('Algerian',20))
stopper=PhotoImage('D:\\download.gif')
canvas.create_image(200,200,anchor=NW,image=stopper)
...
while 1:
    ...
    if ball.hitbottom==True:
        ...
        #canvas.itemconfig(stopper,state='normal')
        time.sleep(1)
        #canvas.itemconfig(stopper,state='normal')
        canvas.itemconfig(gmover,state='normal')
        break
    i+=0.008
    a.update_idletasks()
    a.update()
